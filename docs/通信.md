...
```text
[SMOKE] adc=315 alarm=1 ready=1\r\n
[LCD] rx FAN ok speed=500 le\r\n
[VOICE] QUERY:TEMP\r\n
```
...
```text
AT+MQTTPUB=0,"$sys/<product_id>/<device_name>/thing/property/post","{\"id\":\"123\"\,\"version\":\"1.0\"\,\"params\":{\"MQ2\":{\"value\":315}}}",0,0\r\n
```

示例：云端下发打开板载 LED、风扇转速设为 `500`、灯带 1 设置为暖色。
...
示例：STM32 将温度控件 `t0` 写为 `25.5`。

```text
t0.txt="25.5" FF FF FF
```

示例：把灯带 1 设置为暖色 `R=255 G=200 B=100`。

```text
55 AA 04 01 FF C8 64 0D 0A
```

示例：把灯带 2 关闭。

```text
55 AA 04 02 00 00 00 0D 0A
```

示例：风扇设置为 `500`，大端写法。

```text
55 AA 05 01 F4 0D 0A
```

示例：风扇设置为 `500`，USART HMI `prints h0.val,2` 常见小端写法，代码同样兼容。

```text
55 AA 05 F4 01 0D 0A
```
