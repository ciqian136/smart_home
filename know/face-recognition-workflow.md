...
- 收到 1 次 `FACE:ZENG` 且分数 >= 70 后，`face_is_zeng_detected()` 返回 1。
- 连续 3 次 `FACE:NONE` 或超过 2000ms 未收到有效帧后，识别状态清零。
- 人脸模块只产生状态和事件，不直接控制执行器；确认识别后由 `voice.c` 先发送欢迎播报，随后 `automation.c` 再执行“打开室内灯/打开风扇”的回家动作。
- 人脸回家动作不受温度、湿度、光照影响，无论环境如何都会强制打开室内灯和风扇。
- 除人脸识别自动开启外，不执行任何其他自动化逻辑；风扇和灯光只允许 app/云端、LCD、语音手动控制，人脸离开后也不会自动关闭。
- 确认识别后由 `voice.c` 发送动态欢迎播报：欢迎回家曾先生、当前温度/湿度、已为您打开风扇、当前光照、已为您打开室内灯。
- 欢迎播报使用 5 秒冷却：持续识别到人脸期间按冷却周期重复播报（face.c 不再只在识别上升沿触发一次事件），同时避免人脸短时间离开/重入时过频打断其他语音交互。
- ASRPRO 外部播报路径对 `PLAY/PLAYS` 增加 45 秒唤醒保活；播放启动失败会快速跳过，播放完成等待约 5 秒超时，超时时主动停止提示音播放。`resume_voice_in()` 只在整批队列清空且批量接收结束后调用一次，避免长欢迎播报中途恢复语音输入导致后续片段被跳过或语音控制暂时失效。
- ASRPRO 的 `app_uart()` 每轮会尽量读完串口缓冲，避免 9600 波特率下长 `PLAYS` 丢字节；解析 `PLAYS` 时会置位批量接收标志，防止播放任务提前恢复语音输入。
- `10003` 是欢迎语 `playid`，ASRPRO 侧应走 `prompt_play_by_voice_id()`；`100~135` 是命令词提示音 ID，继续走 `prompt_play_by_cmd_id()`。
